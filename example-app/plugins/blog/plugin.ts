...
import { Plugin, Container, PluginConfig, ViewEngine, Logger, EventEmitter, DatabaseDriver, WorkerManager } from "../../../engine/mod.ts";
  ...
  async initialize(): Promise<void> {
    ...
    if (this.container.has("db")) {
      ...
      try {
        ...
        logger.warn("Could not create posts table", { error: error.message });
      }
    }
  ...
  async createPost(data: {
    ...
    if (this.container.has("db")) {
      ...
      try {
        ...
        const logger = this.container.resolve<Logger>("logger");
        logger.error("Failed to save post to database", { error: error.message });
      }
    }
