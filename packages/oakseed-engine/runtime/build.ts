...
export class AssetBuilder {
  ...
  constructor(options: BuildOptions = {}, logger?: Logger) {
    this.options = {
    ...
    this.logger = logger || new ConsoleLogger("info", "[AssetBuilder]", true);
  }
  ...
  async watch(): Promise<void> {
    this.logger.info("👀 Watching " + this.options.entryPoints?.join("\n").replaceAll('"', ''));
...
if (import.meta.main) {
  ...
  const appMain = new AssetBuilder({
    outdir: outDir,
    watch: watch,
  ...
  if (watch) {
    await appMain.watch();
    if (includePlugins) {
      await AssetBuilder.buildAll("../packages/oakseed-engine/plugins", pluginOutDir);
    ...
    if (includePlugins) {
      await AssetBuilder.buildAll("../packages/oakseed-engine/plugins", pluginOutDir);
      await AssetBuilder.buildAll(sourceDirPlugins, pluginOutDir);
    }
  }
}