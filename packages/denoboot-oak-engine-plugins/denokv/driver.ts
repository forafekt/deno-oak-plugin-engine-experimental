...
import type { CacheDriver, DatabaseDriver } from "@denoboot/types";
...
export class DenoKVDriver implements DatabaseDriver, CacheDriver {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Deno KV connection failed", { error: (error as Error).message });
      throw error;
    }
  }
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Deno KV disconnect failed", { error: (error as Error).message });
      throw error;
    }
  }
  ...
  async clear(): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Deno KV clear failed", { error: (error as Error).message });
      throw error;
    }
  }
